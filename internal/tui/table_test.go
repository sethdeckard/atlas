...
func TestChooseColumns_FullWidthKeepsAll(t *testing.T) {
	...
	cols := chooseColumns(200, repos, tableOpts{root: "/r", groupBy: "none"})
	if len(cols) != 4 {
...
func TestChooseColumns_DropsBranchOnNarrow(t *testing.T) {
	...
	cols := chooseColumns(20, repos, tableOpts{root: "/r", groupBy: "none"})
...
func TestRenderTable_DropsRowsOutsideViewport(t *testing.T) {
	...
	out := renderTable(repos, tableOpts{root: "/r", groupBy: "none"}, 2, 1, 2, 80, s)
